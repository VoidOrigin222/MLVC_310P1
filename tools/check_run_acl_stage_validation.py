#!/usr/bin/env python3
"""Regression checks for the ACL stage-validation acceptance report."""
...
def make_fake_compare(path: Path) -> None:
    path.write_text(
        "#!/usr/bin/env python3\n"
        "import sys\n"
        "stage = sys.argv[sys.argv.index('--stage') + 1]\n"
        "print(f'stage={stage}')\n"
        "print('output=y elements=1 mismatches=0 max_abs=0 max_rel=0')\n"
        "print('status=ok')\n",
        encoding="utf-8",
    )
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="mlvc_acl_stage_validation_") as temp:
        root = Path(temp)
        ...
        make_fake_compare(fake_compare)
        output = root / "acceptance" / "single_stage_correctness.json"
        command = [
            sys.executable,
            str(Path(__file__).resolve().parent / "run_acl_stage_validation.py"),
            "--root",
            str(root),
            "--resolution",
            "720p",
        ...
        assert_true(report["gate"] == "single_stage_correctness", "bad gate")
        assert_true(report["validated_stage_count"] == 1, "stage was not validated")
        assert_true(report["results"][0]["metrics"]["outputs"][0]["mismatches"] == 0, "bad metrics")

    print("run_acl_stage_validation_check status=ok")
    return 0
